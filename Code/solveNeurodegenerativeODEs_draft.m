function solveNeurodegenerativeODEs_draft()
    ...
    % Plot the density of extracellular Aβ over time
    figure
    plot(t, solution(:, 6), 'c')
    hold on
    plot(t, solution(:, 7), 'y')
    ...
    xlabel('Time (days)')
    ylabel('Density (g/ml)')
    title('Extracellular Aβ over time')
    legend('Aoβ', 'M1', 'M2')
    hold off

    % Plot the inflammation level over time
    figure
    plot(t, solution(:, 7) ./ solution(:, 8), 'r')
    xlabel('Time (days)')
    ylabel('M1/M2 ratio')
    title('Inflammation level over time')

    % Plot the correlation between neuron density and intracellular NFTs
    figure
    scatter(solution(:, 1), solution(:, 3), 'r')
    xlabel('Neuron density (g/ml)')
    ylabel('Intracellular NFTs (g/ml)')
    title('Correlation between neuron density and intracellular NFTs')
end
